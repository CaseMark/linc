...
describe("InteractiveMode compaction events", () => {
	...
	test("restores the working loader when the same agent run resumes after compaction", async () => {
		...
		const fakeThis = {
			...
			createWorkingLoader: vi.fn().mockReturnValue(loader),
		...
		// A loader that is already showing is left alone on later turns.
		await handleEvent.call(fakeThis, { type: "turn_start" });
		expect(fakeThis.createWorkingLoader).toHaveBeenCalledTimes(1);

...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "linc-local-release-"));
	}
...
function createLincShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "linc.cmd"))) {
			writeFileSync(join(installDirectory, "linc.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\linc.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "linc.ps1"), '& "$PSScriptRoot/node_modules/.bin/linc.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "linc.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pi.exe" %*\r\n');
		writeFileSync(join(installDirectory, "linc.ps1"), '& "$PSScriptRoot/node_modules/.bin/pi.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "linc"), join(installDirectory, "linc"));
}
...
if (rootPackageJson.name !== "linc-monorepo") {
...
if (!options.skipInstall) {
	...
	createLincShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createLincShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "linc.cmd" : "linc")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "linc.cmd" : "linc")} --help`);
	}
}
